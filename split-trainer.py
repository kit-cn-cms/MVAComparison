...
class Trainer:
    def __init__(self, variables, variables_to_try=[],Name , verbose=False):
        ...
        self.rootfile='outfile/autotrain'+self.Name+'.root'
    ...
    def evaluateLastTraining(self):
        ...
        logfile = open("log"+self.Name+".txt","a+")
    ...
    def DivEtImp(NTREES, SHRINKAGE, NCUTS):
      self.setBDTOption("NTrees="+NTREES)
      self.setBDTOption("Shrinkage="+SHRINKAGE)
      self.setBDTOption("nCuts="+NCUTS)
      
      self.trainBDT([],"")
      ROC, ksS, ksB, ROCT = self.evaluateLastTraining()
      
      
      f1=open("data"+self.Name+".dat","w")
      f1.write("ROC="+ROC+"\nROCT="ROCT+"\nksS="+ksS+"\nksB="+ksB)
      f1.close()
...
#for k in range(0,Schritte):
  ...
  #ncuts[k]=nCuts_min+k*int((nCuts_max-nCuts_min)/Schritte)
	...
	for i in range(0,1):
	  #ncuts[i]=nCuts_min+i*int((nCuts_max-nCuts_min)/Schritte)
	  #self.setBDTOption("nCuts="+str(nCuts_min+i*int((nCuts_max-nCuts_min)/Schritte)))
	  ...
	  for k in range(0,Schritte):
	    ...
	    for j in range(0,Schritte):
	      #ntrees[j]=NTrees_min+j*int((NTrees_max-NTrees_min)/Schritte)
	      #self.setBDTOption("NTrees="+str(NTrees_min+j*int((NTrees_max-NTrees_min)/Schritte)))
